...
public abstract class SSCChildView {
    ...
    /**
     * arrButton.add(new ButtonChildClass("License","LOCK","Đánh dấu video có
     * bản quyền", new EventHandler<ActionEvent>() {
     *
     * @Override public void handle(ActionEvent event) {
     * ObservableList<VideoCloner> arrVideoSelect =
     * FXCollections.observableArrayList();
     * arrVideoSelect.addAll(tvVideos.getSelectionModel().getSelectedItems());
     * if (arrVideoSelect.size() == 0) { ShowErrorMessage.showError(scene, "Chọn
     * danh sách video cần đánh dấu"); return; } new
     * NoteLisenceTask(arrVideoSelect, arrVideos,
     * AppModel.getInstance().getArrMusic(), 1).start(); } }));
     * @return
     */
